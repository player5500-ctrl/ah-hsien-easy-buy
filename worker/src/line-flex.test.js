...
test("Postback 格式只含識別碼與數量，不含價格或 displayText", () => {
    ...
    assert.match(serialized, /action=view_order&groupBuyId=GB001&productId=P001/);
});

test("Flex 商品卡顯示圖片、名稱、規格、售價、單位、截止時間與五種按鈕", () => {
    ...
    const serialized = JSON.stringify(message);
    for (const value of [product.name, product.specs, "NT$ 180", "/ 盒", "收單截止", "1份", "2份", "3份", "取消訂購", "查看我的訂單"]) {
